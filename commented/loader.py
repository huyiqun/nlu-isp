...
class TorchDataset(Dataset):
    ...
    def __init__(self, text, slot, intent, ids):
        self.__text = text
        self.__slot = slot
    ...
    def __getitem__(self, index):
        return self.__text[index], self.__slot[index], self.__intent[index], self.ids[index]
...
class DatasetManager(object):
    ...
    def add_file(self, file_path, data_name, if_train_file):
        ...
        text, slot, intent, ids, anticipation, entropy, dp, intent_dist, entropy_map = self.__read_file(file_path, self.__args.anticipation_size, full_only, if_train_file, self.intent_dist, self.entropy_map)

        if if_train_file:
            self.__word_alphabet.add_instance(text)
            self.__slot_alphabet.add_instance(slot)
            self.__intent_alphabet.add_instance(intent)
            self.intent_dist = intent_dist
        ...
        self.data_entries[data_name] = dp
        self.entropy_vec[data_name] = entropy

        # Serialize raw text and stored it.
        self.__digit_word_data[data_name] = self.__word_alphabet.get_index(text)
        self.anticipation[data_name] = self.__word_alphabet.get_index(anticipation)
        if if_train_file:
    ...
    @staticmethod
    def __read_file(file_path, anticipation_size, full_only, train_file, intent_dist, entropy_map):
        ...
        texts, slots, intents, ids = [], [], [], []
        # stores each individual data point
        text, slot = [], []
        # anticipation
        full_query = dict()
        intent_count = defaultdict(int)
        prefix_intent_count = defaultdict(lambda: defaultdict(int))
        get_id = True
        ...
        with open(file_path, 'r') as fr:
            for line in fr.readlines():
                ...
                if len(items) == 0:
                    ...
                    current_id = items[0]
                    _, _, sub_id = current_id.split("-")
                    if not train_file or not full_only:
                    ...
                    prefix = " ".join(text[1:])
                    if sub_id == "full":
                        prefix = " ".join(text[1:-1])
                        full_query[current_id] = text + ["<PAD>"] * anticipation_size

                    for intent in all_intents:
                        prefix_intent_count[prefix][intent] += 1
                        if sub_id == "full":
        ...
        if not intent_dist:
            freq_dict = {}
            freq_dict_norm = {}
            for p, counter in prefix_intent_count.items():
                freq_dict[p] = {k: float(d)/sum(counter.values()) for k, d in counter.items()}
                normed = {k: float(d)/intent_count[k] for k, d in counter.items()}
                freq_dict_norm[p] = {k: d/sum(normed.values()) for k, d in normed.items()}

            intent_dist = freq_dict_norm
            entropy_map = {prefix: np.sum([-x*np.log2(x) for x in dist.values()]) for prefix, dist in intent_dist.items()}
            entropy_map["<BOS>"] = 999
            entropy_map["<EOS>"] = 999
        ...
        for i in range(len(ids)):
            # print(i)
            id = ids[i]
            *sent_id, sub_id = id.split("-")
            entropy = [entropy_map["<BOS>"]]
            # retrieve anticipation
            if sub_id == "full":
                anticipation = ["<PAD>"] * anticipation_size
            else:
                st = int(sub_id) + 1
                ed = st + anticipation_size
                anticipation = full_query["-".join([*sent_id, "full"])][st : ed]
            # calculate entropy
            for j in range(1, len(texts[i])):
                prefix = " ".join(texts[i][1:j+1])
                if prefix in entropy_map:
                    ent = entropy_map[prefix]
                elif prefix.endswith("<EOS>"):
                    ent = entropy_map["<EOS>"]
                else:
                    n = len(intent_count)
                    ent = -(1./n)*np.log2(1./n) * n
                entropy.append(ent)
            assert len(texts[i]) == len(slots[i]) == len(entropy)
            anticipation_vec.append(anticipation)
            entropy_vec.append(entropy)
            data_entry[id] = DP._make(["-".join(sent_id), sub_id, texts[i], intents[i], slots[i], anticipation, entropy])

        assert len(texts) == len(slots) == len(ids) == len(intents) == len(data_entry) == len(anticipation_vec) == len(entropy_vec)

        return texts, slots, intents, ids, anticipation_vec, entropy_vec, data_entry, intent_dist, entropy_map
    ...
    def batch_delivery(self, data_name, batch_size=None, is_digital=True, shuffle=True):
        ...
        if is_digital:
            ...
            ids = self.ids[data_name]
        dataset = TorchDataset(text, slot, intent, ids)
